...
angular.module('app')
.controller('RollcallCtrl',function(API_URL, $scope, $http, $stateParams, $ionicPopup, $state) {
  var self = this;
    ...
    for (var i = $scope.attendees.length - 1; i >= 0; i--) {
      ...
      for(var j = $scope.rule[0].personTags.length - 1; j >= 0; j--){
        if($scope.attendees[i].tag == $scope.rule[0].personTags[j]){
          $scope.attendees[i].lvl = "black";
          break;
        }
        if(j == 0){
          $scope.showWarning = "block";
        }
      }
      //check payment
  ...
  $scope.toStudentDetail = function(id){
